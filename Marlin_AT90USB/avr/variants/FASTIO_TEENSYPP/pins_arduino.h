...
#define FASTIO_TEENSYPP  2    // so sketch can tell which board was selected in the Arduino IDE
...
#define digitalPinToPCICR(p)    (((p) >= 20 && (p) <= 27) ? (&PCICR) : ((uint8_t *)0))
#define digitalPinToPCICRbit(p) 0
#define digitalPinToPCMSK(p)    (((p) >= 20 && (p) <= 27) ? (&PCMSK0) : ((uint8_t *)0))
#define digitalPinToPCMSKbit(p) ( ((p) >= 20 && (p) <= 27) ? (p) - 20 : (-1))
